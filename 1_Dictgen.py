...
def load_data():
    freeSolve = pickle.load(open('consol.pickle', 'rb')) # Consolidated FreeSolve/Physics Database

    expt, tip, smiles, gbn, igb, asc, zap, cha, nul = [], [], [], [], [], [], [], [], []
    for i in freeSolve.keys():# Transpose Dictionary
        ...
        nul.append(0)
    feats = {'expt' : expt, 'smiles' : smiles, 'tip3p' : tip, 'gbnsr6' : gbn, 'igb5' : igb, 'asc' : asc, 'null' : nul, 'zap9' : zap, 'cha' : cha}
    return feats
...
if __name__ == "__main__":
    ...
    for i in np.vstack((rigid_test_names, rigid_test_BGB)).T:
        if('BGB+' in freesolv[name_to_id[i[0]]].keys()):
            print('duplicate: ', i, name_to_id[i[0]])
